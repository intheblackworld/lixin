...
$time_end        = isset($_POST['time_end']) ? $_POST['time_end'] : '';
    ...
    $mail->Body = "網站：https://" . $src . "/<BR>姓名：" . $name . "<BR>電話：" . $phone . "<BR>公司：".$company."<BR>信箱：".$user_email."<BR>標題：".$title."<BR>留言：".$msg."<BR><BR>填表日期：".$datetime."<BR>廣告來源：".$utm_source."<BR>廣告媒介：".$utm_medium."<BR>廣告名稱：".$utm_campaign."<BR>廣告內容：".$utm_content; //設定郵件內容
    ...
    if ($bCheck == true) { //if start
      ...
      # 添加到 Googlde 資料DB Start
        try {
            ...
            $url .= "&budget=" . $budget;
            $url .= "&message="."標題：".$title."；公司：".$company．"；留言：".$msg;
